...
- **Pure-stdlib Python + curses**, matching the existing foundationhub style:
  screens compose the widgets in `hub/foundationhub/ui.py`; every user-visible
  string goes in `hub/foundationhub/labels.py`; colors only via `foundationhub.theme`
...
- **Hybrid model (decided):** notes, file manager, and system monitor are
  *native Hub screens* inside `hub/foundationhub/`; media and games are *separate
...
- **Multi-user:** all user data is per-account. Get the active account via
  `foundationhub.session.get_active_account()`; follow the `_user_dir()` pattern in
  `hub/foundationhub/screens/notes.py`. Users must never see each other's data.
...
- **Dev environment:** develop off-device. On Windows use `py` +
  ...
  (writable registry), `FOUNDATIONHUB_USER=<name>` (skip login),
  `FOUNDATIONHUB_DATA=<path>` (data root). Run with `cd hub && py -m foundationhub`.
...
Build:
1. `hub/foundationhub/editor.py` — a reusable curses text-editor widget (not a
...
2. Rework `hub/foundationhub/screens/notes.py` (PERSONAL FILE):
   ...
   - Tags: inline `#tag` tokens in the note body; parser in a testable
     module (e.g. `hub/foundationhub/notesdb.py` — scanning, tag index, search).
...
Build:
1. `hub/foundationhub/screens/files.py` — directory listing (dirs first, sizes,
...
3. Quota readout: header shows used vs. tier allotment (e.g.
   ...
   Tier data: `foundationhub.accounts.TIERS[acct.tier].quota_bytes`.
...
Build:
1. `hub/foundationhub/screens/monitor.py` + a testable `hub/foundationhub/sysinfo.py`
...
## §4 FOUNDATIONMEDIA (separate in-house program)
...
standalone in-house TUI program at top-level `media/foundationmedia/`, launched by
the already-wired Programs MEDIA item (`Launch(["foundationmedia"])`).
...
2. TUI: library view over the account's media dir (per-user!), playlist
   ...
   (factor shared chrome from `foundationhub/ui.py`/`theme.py` rather than
   duplicating — acceptable to import foundationhub as a library).
3. Install wiring: new `media/` dir installed like `hub/` (see
   `install/04-hub.sh` pattern); add a bin shim `foundationmedia`.
...
**Recommended models:** foundation_arcade → Sonnet 5 (classic game loops,
...
`system/etc/foundationhub/recreation.toml` stays the registry — games are config
entries, not Hub code.

Build order (operator said fine to split across chats):
1. **foundation_arcade** — one program, several small games, shared engine
   (top-level `games/foundation_arcade/`): Snake, a falling-blocks game, 2048,
